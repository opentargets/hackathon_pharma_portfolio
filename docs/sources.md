...
| Merck KGaA | `https://www.merckgroup.com/en/research/healthcare-pipeline.html` | **Downgraded from Tier 3.** No WAF, no click-loop needed — the widget's own script loads a plain JS data file (`data.js`) directly fetchable via curl. 10 rows across 3 areas, merged with the site's own pipeline PDF for footnote-level detail (license deals, regulatory status). See [`src/pharmas/merck_kgaa/log.md`](../src/pharmas/merck_kgaa/log.md) | Done |
